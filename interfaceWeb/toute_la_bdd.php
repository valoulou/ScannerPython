<html>
    ...
    <body>
      ...
      $result = $bdd->query ("SELECT machines.ip, services.mid, services.sid, services.port, services.proto, services.nom_service, services.state, services.version, services.last_view, services.manage  FROM services, machines WHERE (machines.mid = services.mid) AND state = 'open'");
            ...
            <td>
              <form action="script.php" method="post">
                <select type="text" name="manageValue" size="1" />
                  <option></option>
                  <option value=<?php echo htmlentities("1;" . $row["mid"] . ";" . $row["sid"] , ENT_QUOTES, "utf-8");?>>Laisser Ouvert</option>
                  <option value=<?php echo htmlentities("2;" . $row["mid"] . ";" . $row["sid"] , ENT_QUOTES, "utf-8");?>>Le Fermer</option>
                </select>
